...
1. **`tanks.py`**: This is the main game file. It contains the implementation of the game logic, including tank movement, shooting mechanisms, and the game loop. This file initializes a window using `graphics.py` that contains both the home page and gameplay screen. During gameplay, it (1) checks for user input to move the tank or shoot missiles. (2) navigates the CPU tanks in effort to destory the user (3) checks for collisions between missiles and tanks to determine if a tank is destroyed, (3) updates levels and keeps track of multiple ongoing factors, (4) and much more.
